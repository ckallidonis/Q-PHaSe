...
# and calculate the scalar charge.
#
...
optr_tag = 'scalar' # We are computing the scalar charge
obs_tag = 'scalar_charge'
#--------------------------------------------------------------------------------
...
if( Qsq2pt != Qsq3pt ):
    print('scalar_charge.py: Q-square lists for two- and three-point functions do not agree! Exiting.')
    sys.exit()
...
if (twopInfo['traj_list'] != thrpInfo['traj_list']):
    print('scalar_charge.py: Trajectory lists for two- and three-point functions do not agree! Exiting.')
    sys.exit()
    
print('3pt: Ntraj = %d, Ntins = %d' % (thrpInfo['Ntraj'],thrpInfo['T']))
#--------------------------------------------------------------------------------

thrp = extractMatrixElement(thrp_raw,thrpInfo,Qsq3pt,basis_tag,optr_tag) # Extract the scalar charge from the three-point function
...
for comb in flcombs_conn:
    ...
    for t in range(T):
        for bn in range(Nbins):
            ratio_bin[comb][bn][t] = -thrp_bins[comb][bn][t].imag / twop_bins[bn][tsink].real # Ratio yielding the scalar charge
