...
# and calculate the nucleon sigma-term
#
...
if ( len(sys.argv) != 10 ):
    print('Usage: %s Gamma_basis(QuaHoG, QUDA_UKQCD, QUDA_DeGrand) nsrc msqMax twop-filename thrp-filename tsink m_quark(MeV) binsize output_dir' % (sys.argv[0]))
    sys.exit()
...
optr_tag = 'scalar' # We are computing the sigma-term, coming from the scalar operator
obs_tag = 'sigma-term'
#--------------------------------------------------------------------------------
...
if( Qsq2pt != Qsq3pt ):
    print('sigma-term.py: Q-square lists for two- and three-point functions do not agree! Exiting.')
    sys.exit()
...
if (twopInfo['traj_list'] != thrpInfo['traj_list']):
    print('sigma-term.py: Trajectory lists for two- and three-point functions do not agree! Exiting.')
    sys.exit()
    
print('3pt: Ntraj = %d, Ntins = %d' % (thrpInfo['Ntraj'],thrpInfo['T']))
#--------------------------------------------------------------------------------

thrp = extractMatrixElement(thrp_raw,thrpInfo,Qsq3pt,basis_tag,optr_tag) # Extract the scalar matrix element from the three-point function
...
for comb in flcombs_conn:
    ...
    for t in range(T):
        for bn in range(Nbins):
            ratio_bin[comb][bn][t] = -thrp_bins[comb][bn][t].imag / twop_bins[bn][tsink].real # Ratio yielding the sigma-term

    ratio_bin[comb] *= mq # Multiply with the light quark mass in MeV
